...
| Key | Action |
|-----|--------|
| `<leader>ut` | 打开 / 关闭撤销树浮窗（Neovim 0.12 内置包 `nvim.undotree`，首次按下才加载） |

树窗内光标移动即实时应用对应保存点（所见即所得）；`⏎` 采纳并关闭；`q` / `Esc` 放弃浏览、恢复打开前的保存点；`<leader>ut` 再按一次为直接关闭（保留当前）。配合 `undofile`，关掉重开也能翻历史分支。
...
| `K` | 悬浮文档 |
| `gh` | 诊断 / 悬浮文档 |
| `<leader>cd` | 切换诊断显示（波浪线、当前行诊断文字，带通知） |
| `<C-k>` (插入模式) | 参数签名提示 |
| `<leader>cf` | 格式化代码 |
...
| Key | Action |
|-----|--------|
| `<C-n>` | 未打开时显示文件树；已打开时聚焦；焦点在侧栏时关闭当前视图 |
| `g` (树上) | 唯一切换完整文件树与当前仓库 Git 变动文件树的按键 |
...
| `r` (树上) | 重命名 |
| `d` (树上) | 删除 |
| `y` (树上) | 复制选中项绝对路径到 tmux/系统剪贴板；仍可用 `p` 在树内复制文件 |
| `R` (树上) | 刷新当前文件树或 Git 状态，并显示开始与耗时 |
